...
# within a vector somewhere. 
  ...
  E_N_numerator=(N/EpizooticPotential_round)*-1*Q*FECUNDITY*OMEGA*(GAMMAE*(MUIW+MUID+ALPHA)-GAMMAI*EPSILON)
  E_N_denominator=MUS*(N^(Q+1))*(MUL+OMEGA)*(MUEW+MUED+EPSILON)*(MUIW+MUID+ALPHA)
  E_N=E_N_numerator/E_N_denominator
  
  E_FECUNDITY_numerator=(FECUNDITY/EpizooticPotential_round)*ETA*GAMMAE*OMEGA*(MUIW+MUID+ALPHA)+ETA*GAMMAI*OMEGA*EPSILON+N^Q*SIGMA*OMEGA*(THETAE+PHIE*MUED)*(MUIW+MUID+ALPHA)+N^Q*SIGMA*OMEGA*EPSILON*(THETAI+PHII*MUID+PHIX*ALPHA)
  ...
  E_MUS_numerator=(MUS/EpizooticPotential_round)*-1*GAMMAE*FECUNDITY*OMEGA*(MUIW+MUID+ALPHA)-ETA*GAMMAI*FECUNDITY*OMEGA*EPSILON-SIGMA*FECUNDITY*OMEGA*N^Q*(THETAE+PHIE*MUED)*(MUIW+MUID+ALPHA)-SIGMA*FECUNDITY*OMEGA*EPSILON*N^Q*(THETAI+PHII*MUID+PHIX*ALPHA)
  E_MUS_denominator=MUS^2*ETA*N^Q*(MUL+OMEGA)*(MUEW+MUED+EPSILON)*(MUIW+MUID+ALPHA)
  E_muS=E_MUS_numerator/E_MUS_denominator
  
  E_MUL_numerator=(MUL/EpizooticPotential_round)*-1*GAMMAE*FECUNDITY*OMEGA*ETA*(MUIW+MUID+ALPHA)-GAMMAI*FECUNDITY*OMEGA*EPSILON*ETA-SIGMA*FECUNDITY*OMEGA*N^Q*(THETAE+PHIE*MUED)*(MUIW+MUID+ALPHA)-SIGMA*FECUNDITY*OMEGA*EPSILON*N^Q*(THETAI+PHII*MUID+PHIX*ALPHA)
  E_MUL_denominator=MUS*ETA*N^Q*(MUL+OMEGA)^2*(MUEW+MUED+EPSILON)*(MUIW+MUID+ALPHA)
  E_muL=E_MUL_numerator/E_MUL_denominator
  
  E_MUED_numerator=(MUED/EpizooticPotential_round)*-1*GAMMAE*FECUNDITY*OMEGA*ETA*(MUIW+MUID+ALPHA)-GAMMAI*FECUNDITY*OMEGA*EPSILON*ETA+SIGMA*FECUNDITY*OMEGA*N^Q*(PHIE*(MUEW+MUED+EPSILON)-THETAE-PHIE*MUED)*(MUIW+MUID+ALPHA)-SIGMA*FECUNDITY*OMEGA*EPSILON*N^Q*(THETAI+PHII*MUID+PHIX*ALPHA)
  E_MUED_denominator=MUS*ETA*N^Q*(MUL+OMEGA)*(MUEW+MUED+EPSILON)^2*(MUIW+MUID+ALPHA)
  E_muEd=E_MUED_numerator/E_MUED_denominator
  
  E_MUEW_numerator=(MUEW/EpizooticPotential_round)*-1*ETA*GAMMAE*FECUNDITY*OMEGA*(MUIW+MUID+ALPHA)-ETA*GAMMAI*FECUNDITY*OMEGA*EPSILON-SIGMA*FECUNDITY*OMEGA*N^Q*(THETAE+PHIE*MUED)*(MUIW+MUID+ALPHA)-SIGMA*FECUNDITY*OMEGA*EPSILON*N^Q*(THETAI+PHII*MUID+PHIX*ALPHA)
  E_MUEW_denominator=MUS*ETA*N^Q*(MUL+OMEGA)*(MUEW+MUED+EPSILON)^2*(MUIW+MUID+ALPHA)
  E_muEw=E_MUEW_numerator/E_MUEW_denominator
  
  E_MUID_numerator=(MUID/EpizooticPotential_round)*-1*GAMMAI*FECUNDITY*OMEGA*EPSILON*ETA-SIGMA*FECUNDITY*OMEGA*EPSILON*THETAI*N^Q+SIGMA*FECUNDITY*OMEGA*EPSILON*PHII*N^Q*(MUIW+ALPHA)-SIGMA*FECUNDITY*OMEGA*EPSILON*PHIX*ALPHA*N^Q
  E_MUID_denominator=MUS*ETA*N^Q*(MUL+OMEGA)*(MUEW+MUED+EPSILON)*(MUIW+MUID+ALPHA)^2
  E_muId=E_MUID_numerator/E_MUID_denominator
  
  E_MUIW_numerator=(MUIW/EpizooticPotential_round)*-1*GAMMAI*FECUNDITY*OMEGA*EPSILON*ETA-SIGMA*FECUNDITY*OMEGA*EPSILON*N^Q*(THETAI+PHII*MUID+PHIX*ALPHA)
  E_MUIW_denominator=MUS*ETA*N^Q*(MUL+OMEGA)*(MUEW+MUED+EPSILON)*(MUIW+MUID+ALPHA)^2
  E_muIw=E_MUIW_numerator/E_MUIW_denominator
  
  E_GAMMAE_numerator=(GAMMAE/EpizooticPotential_round)*FECUNDITY*OMEGA
  E_GAMMAE_denominator=MUS*N^Q*(MUL+OMEGA)*(MUEW+MUED+EPSILON)
  E_gammaE=E_GAMMAE_numerator/E_GAMMAE_denominator
  
  E_GAMMAI_numerator=(GAMMAI/EpizooticPotential_round)*FECUNDITY*OMEGA*EPSILON
  E_GAMMAI_denominator=MUS*N^Q*(MUL+OMEGA)*(MUEW+MUED+EPSILON)*(MUIW+MUID+ALPHA)
  E_gammaI=E_GAMMAI_numerator/E_GAMMAI_denominator
  
  E_SIGMA_numerator=(SIGMA/EpizooticPotential_round)*FECUNDITY*OMEGA*(THETAE+PHIE*MUED)*(MUIW+MUID+ALPHA)+FECUNDITY*OMEGA*EPSILON*(THETAI+PHII*MUID+PHIX*ALPHA)
  E_SIGMA_denominator=MUS*ETA*(MUL+OMEGA)*(MUEW+MUED+EPSILON)*(MUIW+MUID+ALPHA)
  E_sigma=E_SIGMA_numerator/E_SIGMA_denominator
  
  E_OMEGA_numerator=(OMEGA/EpizooticPotential_round)*FECUNDITY*MUL*(GAMMAE*ETA*(MUIW+MUID+ALPHA)+GAMMAI*EPSILON*ETA+SIGMA*N^Q*(THETAE+PHIE*MUED)*(MUIW+MUID+ALPHA)+SIGMA*EPSILON*N^Q*(THETAI+PHII*MUID+PHIX*ALPHA))
  ...
  E_EPSILON_numerator=(EPSILON/EpizooticPotential_round)*-1*GAMMAE*FECUNDITY*OMEGA*ETA*(MUIW+MUID+ALPHA)+GAMMAI*FECUNDITY*OMEGA*ETA*(MUEW+MUED)-SIGMA*FECUNDITY*OMEGA*N^Q*(THETAE+PHIE*MUED)*(MUIW+MUID+ALPHA)+SIGMA*FECUNDITY*OMEGA*N^Q*(THETAI+PHII*MUID+PHIX*ALPHA)*(MUEW+MUED)
  ...
  E_ALPHA_numerator=(ALPHA/EpizooticPotential_round)*-1*GAMMAI*FECUNDITY*OMEGA*EPSILON*ETA-SIGMA*FECUNDITY*OMEGA*EPSILON*THETAI*N^Q-SIGMA*FECUNDITY*OMEGA*EPSILON*PHII*MUID*N^Q+SIGMA*FECUNDITY*OMEGA*EPSILON*PHIX*N^Q*(MUIW+MUID)
